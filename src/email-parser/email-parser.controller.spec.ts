...
import { EmailEvent } from '@models/email-event.model'; // Adjust the import based on your project structure
...
describe('EmailParserController', () => {
      ...
      providers: [
        {
          ...
          useValue: {
            parseEmailContent: jest.fn().mockReturnValue(mockParsedContent),
          },
        },
      ],
    }).compile();
  ...
  it('should return parsed email content', async () => {
    jest
      .spyOn(service, 'parseEmailContent')
      .mockImplementation(() => mockParsedContent);

    const result = await controller.parseEmail(mockEmailEvent);
    console.log(
      '🚀 ~ file: email-parser.controller.spec.ts:160 ~ it ~ result:',
      result,
    );

    expect(result).toBe(mockParsedContent);
    expect(service.parseEmailContent).toHaveBeenCalledWith(mockEmailEvent);
  });

  it('should handle errors thrown by the service', async () => {
    const error = new Error('Test Error');
    jest.spyOn(service, 'parseEmailContent').mockImplementation(() => {
      throw error;
    });

    await expect(controller.parseEmail(mockEmailEvent)).rejects.toThrow(error);
  });
});
